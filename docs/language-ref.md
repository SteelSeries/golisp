...
**Vectors** are a more efficient alternative to lists when the number of
  ...
  done explicitly with a function. Many enumeration and access function that
...
**Frames** are sets of named slots that hold arbitrary values, including
  ...
  Frames are inspired by the languages Self [4] and Newtonscript [5].
...
chapter describes the basic GoLisp special forms.
...
of the **lambda** expression are evaluated sequentially in it. This means that
the region of the variables bound by the **lambda** expression is all of the
_sexprs_ in the body. The result of evaluating the last _sexpr_ in the body is
...
Note Iteration::, for information on "named **let**".
...
MIT/GNU Scheme allows any of the _inits_ to be omitted, in which case the
corresponding VARIABLEs are unassigned.
...
The **begin** special form is used to evaluate expressions in a
particular order.
...
### (pred _integer_) ###
 ...
 `(succ z)` is equivalent to `(+ z 1)`; `(pred z)` is equivalent to `(- z 1)`.
...
returned. However, if it is a float the integer part is returned. Note that this
is implemented in go by casting the `float32` to a `int64` so converting
negative floats gives a large integer.
...
Returns whether the first argument is not equal to the second argument. The
following are quivalent:
...
Each _object_ is evaluated; if all of the evaluated _object_s are truthy, returns the value of the final one

### (boolean/or _object_...) ###
<p class="annotation">misc.lsp</p>

Each _object_ is evaluated; if any of the evaluated _object_s are truthy, returns the first one that is
...
Returns the contents of the car field of _pair_. Note that it is an error to
...
Returns the contents of the cdr field of _pair_. Note that it is an error to
...
This copies an arbitrary _tree_ constructed from pairs, copying both the car and
cdr elements of every pair. This could have been defined by
...
Returns a list that is the argument _list_s concatenated together. The arguments
...
applications of **predicate** are made is not specified. One of the returned
...
Returns a newly allocated vector whose elements are the given _object_s.
...
return that pair; the returned pair is always an **_element_** of _alist_,
**_not_** one of the pairs from which _alist_ is composed. If no pair in _alist_
...
Notice that we’ve been saying parent **_frames_**, i.e. plural. Also note that
...
Writes the concatenation of the string forms of _object_s followed by a newline.
...
concatenating the string forms of all the _object_s.
...
3. The Go function which implements the primitive. This function **must** have
   the signature `func <Name>(*Data, *SymbolTableFrame) (*Data, error)`
...
you are hackign on the language itself (as opposed to adding builting
functions):
...
implementing function has full control over what gets evaluated and when.
...
**PairP(*Data) bool** returns whether the data is a cons cell whose cdr is nil
  or another cons cell.

**DottedPairP(*Data) bool** returns whether the data is a cons cell is somethign
  other than nil or a cons cell.

**ListP(*Data) bool** returns whether the data is a proper list. E.g.  (a b c d)

**DottedListP(*Data) bool** returns whether the data is a dotted list. E.g. (a b
  c . d)

**CircularListP(*Data) bool** returns whether the data is a list that contains a
  loop. I.e the final cdr refers to one of the pairs in the list.

**ObjectP(*Data) bool** returns whether the data is an encapsulated Go object

**FunctionP(*Data) bool** returns whether the data is either a user defined or
  primitive function
...
**IntegerWithValue(n int64) *Data** creates a number (integer) with the provided
value

**FloatWithValue(n float32) *Data** creates a number (float) with the provided
value
...
**ObjectWithTypeAndValue(typeName string, o unsafe.Pointer) *Data** creates an
encapsulated Go object with the given type tag and value
...
**Length(*Data) int** returns the length of a piece of Lisp data. This will be 0
in all cases except lists, when it will be the number of elements in the list.
...
- if the data are lists, they are equal if corresponding pairs of elements are
  equal
...
[6] Chris Hanson and the MIT Scheme Team. *MIT/GNU Scheme Reference Manual*, Edition 1.105 for release 9.2.  Massachusetts Institute of Technology. May 2014.
