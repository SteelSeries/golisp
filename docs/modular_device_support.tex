...
placed in the result. To do this you provide a count using a \verb|repeat| clause. As an example, here's a declaration that
...
All leaf field are a number with a valid range given by it's size: 0x00-0xFF for uint8, 0x0000-0xFFFF for uint16, and 0x00000000-0xFFFFFFFF for uint32. For some fields the entire range isn't valid. To enforce this, we introduce validation clauses to the field definition. These take two types: ranges and values. A range is specified by lower and upper bounds, which are inclusive. A set of values is just that: a list of valid values for the field.
...
When this jason is received by the device support layer, it gets processed and converted to a Lisp structure:
