...
sexprs) the \verb|payload| symbol available that evaluates to the
serialized payload as defined by the corresponding structure using the
provided json data.
...
both a structure and an api. The data at that key is used to build the
payload bytearray from the appropriate structure which is available to
the API read/write commands.
