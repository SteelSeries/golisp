...
prefix and suffix. The characters \verb|?|, \verb|!|, and \verb|*| are
used as the final character of a function name to denote:

\begin{description}
\item [?] a predicate, e.g. \verb|nil?|
\item [!] a mutating function (changes the argument rather than
  returning a modified copy), e.g. \verb|set!|
\item [*] a varient of the main function, e.g. \verb|flatten| and
  \verb|flatten*| (which is a recursive \verb|flatten|)
\end{description}
...
\item[Integers] ar enon-negative integers internally represented as \verb|uint32| so
...
\item[Floats] are Go \verb|float32| numbers. Accordingly they are
  signed. All arithmetic functions (modulus is a bit different) work
  as expected for both numbers and floats. Numbers are coerced to
  floats as required, specifically is any arguments are float, all are
